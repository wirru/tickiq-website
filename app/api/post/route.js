    ...
    if (supabaseUrl && supabaseAnonKey && postId !== 'post') {
      ...
      if (response.ok) {
        ...
        if (data.image_token) {
          ...
          ogImageUrl = imageUrl;
          // Landing page uses the same image
...
<body data-campaign-token="web-post">
    ...
    <script>
        ...
        // Update download button with campaign-tracked URL
        if (downloadButton) {
            ...
            downloadButton.href = \`https://apps.apple.com/app/apple-store/id6749871310?\${params.toString()}\`;
        }
